 ...
 */
class Tukish_CmsHierarchy_Model_Resource_Cms_Page extends Mage_Cms_Model_Mysql4_Page
{
    ...
    protected function _construct()
    {
        ...
        Mage::getSingleton('core/resource')->setMappedTableName('cms_page', $tablePrefix . 'tukish_cms_page_tree');
        if ($this->_tableTreeStoreExists()) {
            Mage::getSingleton('core/resource')->setMappedTableName(
                'cms_page_store',
                $tablePrefix . 'tukish_cms_page_tree_store'
            );
        }
    }
    ...
    {
        Mage::app()->cleanCache(array(Tukish_CmsHierarchy_Block_Catalog_Navigation::CACHE_TAG));
    ...
    protected function _tableTreeStoreExists()
    {
        if (null === $this->_tableTreeStoreExists) {
            $this->_tableTreeStoreExists = $this->getReadConnection()
                ->isTableExists(Mage::getConfig()->getTablePrefix() . 'tukish_cms_page_tree_store');
        }
